...
class DashboardController extends Controller
{
    ...
    public function tdayproducts()
    {
        ...
        $orderedProducts=[];
        foreach ($todays_total_orders as $orders){ 
            $products = unserialize(bzdecompress(utf8_decode($orders->cart)));
            foreach ($products->items as $item){ 
                array_push($orderedProducts, array("product" => $item['item']['name'], "qty" => $item['qty']));
            }  
        }
        $result = [];
        ...
        return view('admin.order.singleDayOrder',compact('result'));
    }
